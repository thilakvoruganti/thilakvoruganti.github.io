...
const cards = [
  ...
  },
  {
    label: 'Dribbble',
    image: IMG_DRIBBBLE,
    title: 'Explore Dribbble',
    description: 'Visual UI explorations and interface concepts',
                ...
                {card.href ? (
                  ...
                  >
                    <img
                      ...
                      alt=""
                      aria-hidden="true"
                      className="absolute left-0 top-1/2 h-8 w-8 -translate-y-1/2 opacity-0 transition-all duration-200 group-hover:opacity-100 group-focus-visible:opacity-100"
                      loading="lazy"
                      decoding="async"
                    />
                    <span className="transition-all duration-200 group-hover:pl-10 group-focus-visible:pl-10">
                      {card.title}
                    </span>
                  </a>
                ) : (
                  <div className="text-white text-[32px] leading-[39px] font-semibold">{card.title}</div>
                )}
